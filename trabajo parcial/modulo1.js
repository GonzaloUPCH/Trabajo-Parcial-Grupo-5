...
function applyTheme() {
  const saved = localStorage.getItem("theme_modulo1");
...
function toggleTheme() {
  ...
  localStorage.setItem("theme_modulo1", next);
...
function validatePatient(payload) {
  ...
  if (duplicateDoc) {
    errors.push("El documento de identidad ya está registrado.");
  }
  ...
  if (!payload.alergias.length) {
    errors.push("Debe seleccionar al menos una alergia.");
  }
  ...
  if (payload.edad < 18) {
    if (!payload.apoderado || payload.apoderado.length < 3) {
...
function savePatient() {
  ...
  if (editingCode) {
    patients = patients.map((patient) =>
      patient.codigo === editingCode ? patientRecord : patient
    );
      ...
      <article class="patient-card">
        ...
        <div class="patient-info">
          ...
          <div class="patient-info-row">
            ...
            <span>${patient.correo || "No registrado"}</span>
          </div>
          <div class="patient-info-row">
          ...
          <div class="patient-info-row">
            ...
            <span>${patient.apoderado || "No aplica"}</span>
          </div>
          <div class="patient-info-row">
            <strong>Emergencia</strong>
            <span>${patient.emergenciaNombre} (${patient.emergenciaParentesco}) - ${patient.emergenciaTelefono}</span>
          </div>
        </div>
        ...
        <div class="patient-actions">
          <button type="button" class="btn btn-secondary" onclick="editPatient('${patient.codigo}')">Editar</button>
        </div>
      </article>
    `)
    .join("");
